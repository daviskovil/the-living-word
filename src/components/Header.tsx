...
export function Header({ streak, milestoneMessage }: HeaderProps) {
  ...
  return (
    <header className="flex items-start justify-between mb-10">
      ...
      <div className="flex flex-col items-end gap-1.5">
        {streak > 0 && (
          <span
            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-orange-50 border border-orange-200 text-xs font-semibold text-orange-700 ${
              milestoneMessage ? "animate-pulse-gentle" : ""
            }`}
          >
        ...
        {milestoneMessage && (
          <span className="text-xs text-accent-600 font-medium animate-fade-in">
            {milestoneMessage}
          </span>
        )}
      </div>
